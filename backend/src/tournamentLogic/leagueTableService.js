...
export const generateLeagueTable = async (tournamentId) => {
  ...
  completedFixtures.forEach(fixture => {
    ...
    tableData[homeId].goalDifference = tableData[homeId].goalsFor - tableData[homeId].goalsAgainst;
    tableData[awayId].goalDifference = tableData[awayId].goalsFor - tableData[awayId].goalsAgainst;
...
export const getTeamPosition = async (tournamentId, teamId) => {
  ...
  const context = {
    above: teamIndex > 0 ? fullTable.table.slice(Math.max(0, teamIndex - 2), teamIndex) : [],
    team: team,
    below: teamIndex < fullTable.table.length - 1 ? 
           fullTable.table.slice(teamIndex + 1, Math.min(fullTable.table.length, teamIndex + 3)) : [],
  };
